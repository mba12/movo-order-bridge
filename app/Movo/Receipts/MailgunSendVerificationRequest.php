...
class MailgunSendVerificationRequest extends Verification implements VerifyInterface
{
    public function send(array $data)
    {
        ...
        $data['link'] = 'https://orders.getmovo.com/verify/user-confirm?action=verify&trackingnumber=' . $data['key'] . '&id=' . $data['id'];

        Mail::send('emails.sendverification', array('data' => $data), function ($message) use ($data) {
            $message->to($data['email'], $data['fullName'])->subject('Movo Email Verification')->from("no_reply@getmovo.com");
        });
